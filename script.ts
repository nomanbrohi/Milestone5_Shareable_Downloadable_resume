...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const editCvButton = document.getElementById('editCv') as HTMLElement;
    const downloadCvButton = document.getElementById('downloadCv') as HTMLButtonElement; // New download button
    ...
    educationInstitutions.forEach((institution, index) => {
      ...
      // Add institution as a heading (h5)
      const institutionHeading = document.createElement('h5');
      ...
      softwareSkills.forEach((skill) => {
        ...
        editCvButton.classList.remove('none');
  
      });

      const editaleButton = document.getElementById('editCv') as HTMLButtonElement;

      editaleButton.addEventListener("click", () =>{
        cv.classList.add('none');
        cvmaindiv.classList.remove('none');
        editCvButton.classList.add('none');
      });

      downloadCvButton.addEventListener('click', () => {
        const cvElement = document.getElementById('cv') as HTMLElement; // The div containing the CV content
